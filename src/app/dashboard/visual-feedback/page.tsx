...
import { UploadCloud, MessageSquare, X, Send } from 'lucide-react';
...
export default function VisualFeedbackPage() {
    ...
    const handleImageClick = (e: MouseEvent<HTMLDivElement>) => {
        ...
        if (pendingComment) return; // Don't create new comment if one is already pending
    ...
    const handlePostComment = () => {
        if (!pendingComment || !activeImageId || !pendingComment.text.trim()) return;

        const newComment: Comment = {
            id: Date.now().toString(),
            ...pendingComment,
            user: { name: "Designer" }, // Mock user
    ...
    const renderComment = (comment: Comment | { x: number; y: number; text: string }, index: number) => {
        ...
        return (
            ...
            >
                <div className="relative">
                    ...
                    {/* Comment Form for Pending */}
                    {isPending && pendingComment && (
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 bg-background p-4 rounded-lg shadow-2xl z-20 space-y-2">
                             <Textarea 
                            ...
                            <div className="flex justify-end gap-2">
                                <Button variant="ghost" size="icon" onClick={() => setPendingComment(null)}>
                                    <X className="h-4 w-4" />
                                </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 h-[calc(100vh-8rem)]">
                        ...
                        <div className="space-y-2">
                            {images.map(img => (
                                <div 
                                    ...
                                    className={cn(
                                        "p-2 rounded-lg cursor-pointer border-2",
            ...
            <div className="lg:col-span-2 bg-muted/50 rounded-lg flex items-center justify-center relative overflow-hidden">
                {activeImage ? (
                    ...
                    >
                        ...
                        {activeImage.comments.map((comment, index) => renderComment(comment, index))}
                        {pendingComment && renderComment(pendingComment, activeImage.comments.length)}
                    </div>
                ) : (
                    <div className="text-center text-muted-foreground">
                        <MessageSquare className="h-16 w-16 mx-auto mb-4" />
                        <h3 className="text-lg font-semibold">No image selected</h3>
                        <p>Upload or select an image to start leaving feedback.</p>
                    </div>
                )}
            </div>
            ...
            <Card className="lg:col-span-1 flex flex-col">
                <CardHeader>
                    ...
                    <CardDescription>Feedback for the active project appears here.</CardDescription>
                </CardHeader>
                <CardContent className="flex-grow">
                     <ScrollArea className="h-full">
                        <div className="space-y-4">
                            {!activeImage && (
                                 <div className="flex items-center justify-center h-full text-muted-foreground">
                                    <p>Select an image to see comments.</p>
                                 </div>
                            )}
                            {activeImage && activeImage.comments.length === 0 && (
                                <div className="flex items-center justify-center h-full text-muted-foreground">
